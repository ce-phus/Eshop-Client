...
const LaptopDetailsPage = ({ addToCart}) => {
  ...
  useEffect(()=>{
    ...
    dispatch({
      type: UPDATE_LAPTOP_RESET
    });
    dispatch({
      type: CREATE_LAPTOP_RESET
    });
    dispatch({
            ...
            {error ? <Error variant='danger'>{error}</Error> :
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    <div className='max-w-md bg-gray-800 border border-gray-700 rounded-lg shadow'>
                        <img className='rounded-t-lg w-full' src={fullImageUrl} alt={laptop.name} />
                        {userInfo && userInfo.admin &&
                            <div className='flex justify-between p-4'>
                                ...
                                <button className='bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded' onClick={() => router(`/product-update/${laptop.id}/`)}>Edit Product</button>
                            </div>
                        }
                    </div>
                    <div>
                    ...
                    <div className="modal-content py-4 text-left px-6">
                        ...
                        <div className="modal-body py-4 text-gray-500">
                            <p className="text-base leading-relaxed">
                                Are you sure you want to delete this product <span className="font-bold">{laptop.name}</span>?
                            </p>
                        </div>
