...
export const mpesastkpush = (payload, item_type, id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`https://cephuseshop.co.ke/payments/checkout/${item_type}/${id}/`, payload, config);
...
export const mpesaCallbackHandler = (callbackData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post('https://cephuseshop.co.ke/api/callback/', callbackData, config);
