...
export const mpesastkpush = (payload, item_type, id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`http://51.222.174.97:8000/payments/checkout/${item_type}/${id}/`, payload, config);
...
export const mpesaCallbackHandler = (callbackData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post('http://51.222.174.97:8000/api/callback/', callbackData, config);
