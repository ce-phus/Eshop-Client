...
export const getComputerPartsList = () => async (dispatch) => {
    try {
        ...
        // call api
        const { data } = await axios.get("http://51.222.174.97:8000/api/computerparts/", {
            method: 'POST',
            mode: 'cors',
            headers:{
...
export const getComputerPartsDetails = (id) => async (dispatch) =>{
    try {
        ...
        // call api
        const { data } = await axios.get(`http://51.222.174.97:8000/api/computerparts/${id}/`)
        dispatch({
...
export const createcomputerparts = (parts) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(
            "http://51.222.174.97:8000/api/computerparts-create/",
            parts,
            config
        )
...
export const deletecomputerparts = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.delete(
            `http://51.222.174.97:8000/api/computerparts-delete/${id}/`,
            config
        )
...
export const updatecomputerparts = (id, parts) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.put(
            `http://51.222.174.97:8000/api/computerparts-update/${id}/`,
            parts,
            config
        )
